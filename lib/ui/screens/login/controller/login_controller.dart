  ...
  void handleLogin(BuildContext context) {
    ...
    if (error == null) {
      context.read<AuthBloc>().add(
            SendSmsAuthEvent(
              isLogin: true,
              phoneNumber: isPhoneLogin ? phoneController.text : null,
              email: isPhoneLogin ? null : emailController.text,
            ),
          );
      LoginValues.isRegister = true;

      if (isPhoneLogin) {
        LoginValues.phoneNumber = phoneController.text;
      } else {
        LoginValues.email = emailController.text;
      }
    }
  }
}
